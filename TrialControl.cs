...
using UnityEngine;
/* Yale Students Imersive Media Copyright January, 16th 2020
 */

public class TrialControl : MonoBehaviour
{
    public GameObject Return;
    public GameObject plant, pot, spiders, spiderArea,waterArea, panel; // PUT INT SPIDER AREA
   ...
   // public GameObject[] pots = new GameObject[numTrials];
    ...
    public void passTrial(int seed)
    {
        ...
        if (redOrBlue[chooseColor])
        {
           ...
           switch(rLocation[location])
            {
                case 0: plant.transform.position = spot1.position;
                    //print("0");
                    break;
                case 1: plant.transform.position = spot2.position;
                    //print("1");
                    break;
                case 2: plant.transform.position = spot3.position;
                   /// print("2");
                    break;
            }
    ...
    IEnumerator spiderTrialChange()
    {
        ...
        yield return new WaitForSeconds(1f);
            trial++;
            passTrial(trial);
            spiders.SetActive(false); // set spiers to nothing when using other trial!
